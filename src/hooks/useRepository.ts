...
export function useRepository(): UseRepositoryResult {
  ...
  const updateProfile = useCallback(
    (id: string, draft: ProfileDraft): void => {
      persist(
        userProfiles.map((p) =>
          p.id === id ? draftToProfile(draft, id, p.createdAt) : p
        )
      );
    },
    [userProfiles, persist]
  ...
  const replaceProfiles = useCallback((profiles: RepoProfile[]): void => {
  ...
  useEffect(() => {
    function handleStorage(event: StorageEvent) {
      if (event.key === "mil_browser_profiles_v2") {
